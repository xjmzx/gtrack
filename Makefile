...
DESKTOP_OUT := $(APPDIR)/gtrack.desktop
TAURI_BIN   := src-tauri/target/release/gtrack

.PHONY: help deps dev build install uninstall check test clean icons install-guard
...
install: install-guard $(TAURI_BIN)
	...
	install -m 0755 $(TAURI_BIN) $(BINDIR)/gtrack
	@# Linux fill: crop the grid margin on the way in (see LINUX_VIEWBOX).
	sed '1s|viewBox="[^"]*"|viewBox="$(LINUX_VIEWBOX)"|' icon.svg > $(ICONDIR)/gtrack.svg
	chmod 0644 $(ICONDIR)/gtrack.svg
	sed -e 's|@BINDIR@|$(BINDIR)|g' \
	    -e 's|@ICONDIR@|$(ICONDIR)|g' \
	    gtrack.desktop.in > $(DESKTOP_OUT)
	...
	@echo "  binary  -> $(BINDIR)/gtrack"
	@echo "  desktop -> $(DESKTOP_OUT)"

uninstall:
	rm -f $(BINDIR)/gtrack
	rm -f $(ICONDIR)/gtrack.svg
	rm -f $(DESKTOP_OUT)
	@if command -v update-desktop-database >/dev/null 2>&1; then \
		update-desktop-database $(APPDIR) >/dev/null 2>&1 || true; \
	fi
	@# install rebuilt the cache with gtrack.svg in it; left alone, it keeps
